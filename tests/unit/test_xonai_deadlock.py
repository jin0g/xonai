...
class TestXonaiDeadlock:
    ...
    def test_japanese_project_overview_query(self, mock_which, mock_popen):
        ...
        stdout_lines = [
            json.dumps({"type": "system", "subtype": "init", "session_id": "test-123", "model": "claude-3"}),
            json.dumps({"type": "content_block_delta", "delta": {"text": "プロジェクトの概要を把握します。\n"}}),
            json.dumps({"type": "assistant", "message": {"content": [{"type": "tool_use", "name": "LS", "input": {"path": "/"}}]}}),
            json.dumps({"type": "user", "message": {"content": [{"type": "tool_result", "content": "file1.py\nfile2.py\nREADME.md"}]}}),
            json.dumps({"type": "assistant", "message": {"content": [{"type": "tool_use", "name": "Read", "input": {"file_path": "README.md"}}]}}),
            json.dumps({"type": "user", "message": {"content": [{"type": "tool_result", "content": "# Project Title\n" * 100}]}}),  # Large content
            json.dumps({"type": "assistant", "message": {"content": [{"type": "tool_use", "name": "Grep", "input": {"pattern": "class", "path": "."}}]}}),
            json.dumps({"type": "user", "message": {"content": [{"type": "tool_result", "content": "\n".join([f"file{i}.py: class Example{i}" for i in range(50)])}]}}),
            json.dumps({"type": "content_block_delta", "delta": {"text": "このプロジェクトは..."}}),
            json.dumps({"type": "content_block_delta", "delta": {"text": "Pythonで書かれた..."}}),
            json.dumps({"type": "result", "usage": {"input_tokens": 5000, "output_tokens": 2000}, "duration_ms": 5000, "cost_usd": 0.05}),
        ]
    ...
    def test_simulate_actual_deadlock_scenario(self, mock_which, mock_popen):
        ...
        message_count = sum(1 for r in responses if hasattr(r, 'content') and "Processing item" in str(r.content))
